    ...
    <div className="relative z-10 mx-auto max-w-7xl px-4 pb-20 pt-24">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-4 text-4xl font-light text-white sm:text-6xl">全产业链关系图</h1>
          <p className="mt-4 max-w-2xl text-slate-400">初始保持简洁，筛选和点击后显示更高密度的关系。</p>
        </div>
        <DemoDataNotice compact />
      </div>

      <div className="mb-5 flex flex-wrap gap-3">
        <select value={layerFilter} onChange={(event) => setLayerFilter(event.target.value)} className="min-h-11 rounded-md border border-slate-700/35 bg-slate-950/70 px-3 text-sm text-slate-200">
        ...
        <select value={relationshipFilter} onChange={(event) => setRelationshipFilter(event.target.value)} className="min-h-11 rounded-md border border-slate-700/35 bg-slate-950/70 px-3 text-sm text-slate-200">
          ...
          {selectedCompany ? (
            <div className="mt-5">
              <div className="flex items-center gap-3">
                <div className="grid h-12 w-12 place-items-center rounded-md bg-cyan-300/10 font-mono text-xs text-cyan-100">{selectedCompany.logo}</div>
                <div>
              ...
              <Link to={`/companies/${selectedCompany.id}`} className="mt-5 inline-block text-sm text-cyan-100 hover:text-white">
                进入公司详情
              </Link>
          ...
          <button
            ...
            className={`rounded-lg border p-3 text-left transition ${
              selectedCompany?.id === company.id
                ? 'border-cyan-300/45 bg-cyan-300/10 text-white shadow-glow'
                : 'border-slate-700/30 bg-slate-950/35 text-slate-300 hover:border-cyan-300/25'
            }`}
          >
