...
import { sourceTerminalLayerDetails } from '../source-terminal/catalog';
...
type SourceTerminalLayerDetail = {
  id: string;
  description?: string;
  roleInAI?: string;
  whyImportant?: string;
  coreTechnologies: readonly string[];
  technologyBarriers: readonly string[];
  businessModel: readonly string[];
  globalLandscape?: string;
  chinaLandscape?: string;
  localizationRate?: number;
  bottlenecks: readonly string[];
  risks: readonly string[];
  trends: readonly string[];
  upstreamLayerIds: readonly string[];
  downstreamLayerIds: readonly string[];
  companyIds: readonly string[];
};

const sourceLayerDetailsById = new Map(
  (sourceTerminalLayerDetails as readonly SourceTerminalLayerDetail[]).map((details) => [details.id, details]),
);

const asLocaleText = (zh: string, en = zh) => ({ en, zh });

export const layers: Layer[] = layerSeeds.map((seed, index, all) => {
  const sourceDetails = sourceLayerDetailsById.get(seed.id);
  const upstreamLayerIds = sourceDetails?.upstreamLayerIds.length ? [...sourceDetails.upstreamLayerIds] : index > 0 ? [all[index - 1].id] : [];
  const downstreamLayerIds = sourceDetails?.downstreamLayerIds.length
    ? [...sourceDetails.downstreamLayerIds]
    : index < all.length - 1
      ? [all[index + 1].id]
      : [];
  const coreTechnologies = sourceDetails?.coreTechnologies.length ? [...sourceDetails.coreTechnologies] : seed.coreTechnologies;
  const marketLeaders = sourceDetails?.companyIds.length ? [...sourceDetails.companyIds].slice(0, 4) : seed.leaders;
  ...
  return {
    ...
    order: index + 1,
    description: {
      short: { en: seed.short, zh: sourceDetails?.description ?? `${seed.zh}环节决定 AI 扩张中的供给、成本和价值分配。` },
      long: {
        ...
        zh: sourceDetails
          ? `${sourceDetails.roleInAI ?? sourceDetails.description} ${sourceDetails.whyImportant ?? ''}`.trim()
          : `${seed.zh}是 AI 价值链中的关键环节,决定上游资源如何转化为下游能力,并影响成本、供给弹性与定价权。`,
      },
    },
    roleInAI: {
      en: `Provides the ${seed.en.toLowerCase()} capability required for scalable AI systems.`,
      zh: sourceDetails?.roleInAI ?? `为 AI 系统规模化提供${seed.zh}能力。`,
    },
    whyImportant: {
      en: `${seed.en} can become a bottleneck when AI demand expands faster than physical, software or ecosystem capacity.`,
      zh: sourceDetails?.whyImportant ?? `当 AI 需求快于供给扩张时,${seed.zh}会成为决定价值分配的瓶颈。`,
    },
    coreTechnologies,
    technicalBarrier: seed.technicalBarrier,
    businessModel: sourceDetails?.businessModel.length ? [...sourceDetails.businessModel] : ['capacity sale', 'platform service', 'ecosystem lock-in'],
    pricingPower: seed.pricingPower,
    globalLandscape: {
      leaders: marketLeaders,
      marketShare: marketLeaders.map((companyId, leaderIndex) => ({
        companyId,
    ...
    chinaLandscape: {
      ...
      gapToGlobal: {
        ...
        zh: sourceDetails?.chinaLandscape ?? `中国在${seed.zh}环节具备进展,但在工艺深度、生态成熟度或受限关键输入上仍存在差距。`,
      },
      localizationRate: sourceDetails?.localizationRate ?? seed.localizationRate,
      keyBottlenecks: sourceDetails?.bottlenecks.length ? [...sourceDetails.bottlenecks] : ['Top-tier talent density', 'Advanced tooling access', 'Long-cycle validation'],
      policySupport: (sourceDetails?.localizationRate ?? seed.localizationRate) < 0.35 ? 'strong' : 'medium',
    },
    upstreamLayerIds,
    downstreamLayerIds,
    risks: sourceDetails?.risks.length
      ? sourceDetails.risks.map((risk) => ({ type: risk, description: risk }))
      : [
    ...
    futureTrends: sourceDetails?.trends.length
      ? sourceDetails.trends.map((trend) => asLocaleText(trend))
      : [
